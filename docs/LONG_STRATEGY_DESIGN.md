...
- **Stage 3 expected-move test:** implied EM = S·IV·√(DTE/365) ÷ **σ-equivalent** of the median historical |move| (median × 1.4826, the MAD→σ constant) ≤ 1.20 (ideal ≤ 0.80), and EM ≤ p90 historical move. _Recalibrated 2026-09-21: the ratio originally divided by the raw median, but median |move| ≈ 0.67σ for normal-ish returns, so perfectly fair pricing scored ~1.49 and the ≤1.20 gate rejected essentially everything (verified with synthetic normal returns: fair pricing = 1.489 raw, 1.004 scaled). The p75 secondary check had the same defect (fair pricing = 0.87, rejecting at IV/HV ≥ ~1.15) and moved to p90 as an extreme-only guard._
